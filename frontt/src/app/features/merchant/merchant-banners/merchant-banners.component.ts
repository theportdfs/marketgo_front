...
@Component({
  selector: 'app-merchant-banners',
  standalone: true,
  imports: [CommonModule, FormsModule, ImageUploaderComponent],
  template: `
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
      <div class="flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-900">Mis Banners Promocionales</h2>
            ...
            @for (banner of banners; track banner.id) {
              <tr>
                ...
                <td class="px-6 py-4">
                  ...
                  @if (banner.linkUrl) {
                    <div class="text-xs text-indigo-500 mt-1">Enlace: {{ formatLinkTarget(banner.linkUrl) }}</div>
                  }
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
        ...
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div class="bg-white rounded-lg max-w-md w-full p-6 space-y-4">
            ...
            <div class="space-y-3 p-4 bg-gray-50 rounded-lg border border-gray-100">
              <label class="block text-sm font-medium text-gray-700">¿Qué pasa al dar click en el banner?</label>
              <select [(ngModel)]="linkType" (change)="onLinkTypeChange()" class="mt-1 block w-full border rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                <option value="none">Nada (Solo ver imagen)</option>
                <option value="product">Llevar a un Producto</option>
                <option value="bundle">Llevar a un Combo</option>
              </select>

              @if (linkType === 'product') {
                <select [(ngModel)]="linkTargetId" class="mt-2 block w-full border rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                  <option [ngValue]="null">-- Selecciona el Producto --</option>
              ...
              @if (linkType === 'bundle') {
                <select [(ngModel)]="linkTargetId" class="mt-2 block w-full border rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                  <option [ngValue]="null">-- Selecciona el Combo --</option>
...
export class MerchantBannersComponent implements OnInit {
  ...
  linkType: 'none' | 'product' | 'bundle' = 'none';
  ...
  ngOnInit() {
    this.loadBanners();
    this.loadProductsAndBundles();
  }

  loadProductsAndBundles() {
    this.http.get<StoreProduct[]>(`${environment.apiUrl}/api/catalog/store`).subscribe({
      next: (data) => this.products = data
    });
    this.http.get<Bundle[]>(`${environment.apiUrl}/api/bundles`).subscribe({
      next: (data) => this.bundles = data
    });
  }
  ...
  openModal(banner?: Banner) {
    if (banner) {
      ...
      // Parse linkUrl to populate dropdowns
      if (banner.linkUrl?.startsWith('#product-')) {
        this.linkType = 'product';
        this.linkTargetId = parseInt(banner.linkUrl.replace('#product-', ''), 10);
      } else if (banner.linkUrl?.startsWith('#bundle-')) {
        this.linkType = 'bundle';
        this.linkTargetId = parseInt(banner.linkUrl.replace('#bundle-', ''), 10);
      } else {
        this.linkType = 'none';
  ...
  saveBanner() {
    ...
    if (this.linkType === 'none') {
      this.currentBanner.linkUrl = null;
    } else if (this.linkType === 'product' && this.linkTargetId) {
      this.currentBanner.linkUrl = `#product-${this.linkTargetId}`;
    } else if (this.linkType === 'bundle' && this.linkTargetId) {
      this.currentBanner.linkUrl = `#bundle-${this.linkTargetId}`;
    } else {
      this.currentBanner.linkUrl = null;
    }
  ...
  formatLinkTarget(url: string | null): string {
    if (!url) return '';
    if (url.startsWith('#product-')) {
      const id = parseInt(url.replace('#product-', ''), 10);
      const p = this.products.find(x => x.productId === id);
      return p ? `Producto: ${p.product.name}` : 'Producto';
    }
    if (url.startsWith('#bundle-')) {
      const id = parseInt(url.replace('#bundle-', ''), 10);
      const b = this.bundles.find(x => x.id === id);
      return b ? `Combo: ${b.name}` : 'Combo';
    }
    return 'URL Externa';
  }
}
